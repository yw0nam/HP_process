...
csv = csv.query('(fu_1to2 > 180) and (fu_1to2 < 1895)')
csv['final_fu_cdw'] = csv['final_fu_cdw'].fillna('K')
csv['temp'] = csv['fu_dates'] + '|' +  csv['final_fu_cdw']
...
csv['cancer_fu'] = csv.apply(lambda x: None if x['cancer'] == 'Y' and x['cancer_date'] >= '2020-12-31' else x['cancer'], axis=1)
csv['cancer_fu_date'] = csv['cancer_date'].fillna(csv['final_fu_death']).map(lambda x: '2020-12-31' if x >= '2020-12-31' else x)
# %%
csv['cancer_fu_date'] = pd.to_datetime(csv['cancer_fu_date'])
csv['final_fu_death'] = pd.to_datetime(csv['final_fu_death'])
# %%
def check_cancer_or_death(fu, fu_date, first_fu, fu_lenghth=1895):
    """[summary]
...
# %%
csv.to_csv('./../data_for_analysis/any_bx.cov.fillna_cancer.up_death.final_fu.add_fu.csv', index=False)
# %%
